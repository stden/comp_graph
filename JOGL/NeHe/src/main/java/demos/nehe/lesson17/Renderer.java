...
class Renderer implements GLEventListener {
    ...
    private void glPrint(GL2 gl, int x, int y, String string, int set)    // Where The Printing Happens
    {
        if (set > 1) {
            set = 1;
        }
        ...
        stringBuffer.flip();
        gl.glCallLists(string.length(), GL_BYTE, stringBuffer);            // Write The Text To The Screen
        gl.glMatrixMode(GL_PROJECTION);                        // Select The Projection Matrix
    ...
    public void display(GLAutoDrawable glDrawable) {
        ...
        gl.glBindTexture(GL_TEXTURE_2D, textures[1]);            // Select Our Second Texture
