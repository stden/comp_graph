...
class Renderer implements GLEventListener {
    ...
    private int emptyTexture(GL2 gl) {                                            // Create An Empty Texture
        ByteBuffer data = Buffers.newDirectByteBuffer(128 * 128 * 4); // Create Storage Space For Texture Data (128x128x4)
        data.limit(data.capacity());

        int[] txtnumber = new int[1];
        gl.glGenTextures(1, txtnumber, 0);                                // Create 1 Texture
        gl.glBindTexture(GL_TEXTURE_2D, txtnumber[0]);                    // Bind The Texture
        gl.glTexImage2D(GL_TEXTURE_2D, 0, 4, 128, 128, 0,
    ...
    private void processHelix(GL2 gl, GLU glu) {                                // Draws A Helix
        ...
        gl.glMaterialfv(GL_FRONT_AND_BACK, GL_AMBIENT_AND_DIFFUSE, glfMaterialColor, 0);
        gl.glMaterialfv(GL_FRONT_AND_BACK, GL_SPECULAR, specular, 0);

        r = 1.5f;                                                        // Radius

        gl.glBegin(GL_QUADS);                                            // Begin Drawing Quads
    ...
    private void renderToTexture(GL2 gl, GLU glu)                                            // Renders To A Texture
    {
        ...
        gl.glBindTexture(GL_TEXTURE_2D, blurTexture[0]);                    // Bind To The Blur Texture

        // Copy Our ViewPort To The Blur Texture (From 0,0 To 128,128... No Border)
        gl.glCopyTexImage2D(GL_TEXTURE_2D, 0, GL_LUMINANCE, 0, 0, 128, 128, 0);

        gl.glClearColor(0.0f, 0.0f, 0.5f, 0.5f);                        // Set The Clear Color To Medium Blue
        gl.glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);            // Clear The Screen And Depth Buffer
    ...
    private void drawBlur(GL2 gl, int times, float inc)                                // Draw The Blurred Image
    {
        ...
        gl.glBlendFunc(GL_SRC_ALPHA, GL_ONE);                            // Set Blending Mode
        ...
        gl.glDisable(GL_TEXTURE_2D);                                    // Disable 2D Texture Mapping
        gl.glDisable(GL_TEXTURE_2D);                                    // Disable 2D Texture Mapping
        gl.glDisable(GL_TEXTURE_2D);                                    // Disable 2D Texture Mapping
        gl.glDisable(GL_BLEND);                                        // Disable Blending
        gl.glBindTexture(GL_TEXTURE_2D, 0);                                // Unbind The Blur Texture
    }
    ...
    public void init(GLAutoDrawable drawable) {
        ...
        gl.glLightModelfv(GL_LIGHT_MODEL_AMBIENT, lmodel_ambient, 0);        // Set The Ambient Light Model

        gl.glLightModelfv(GL_LIGHT_MODEL_AMBIENT, global_ambient, 0);        // Set The Global Ambient Light Model
        gl.glLightfv(GL_LIGHT0, GL_POSITION, light0pos, 0);                // Set The Lights Position
        gl.glLightfv(GL_LIGHT0, GL_AMBIENT, light0ambient, 0);            // Set The Ambient Light
        gl.glLightfv(GL_LIGHT0, GL_DIFFUSE, light0diffuse, 0);            // Set The Diffuse Light
        gl.glLightfv(GL_LIGHT0, GL_SPECULAR, light0specular, 0);            // Set Up Specular Lighting
