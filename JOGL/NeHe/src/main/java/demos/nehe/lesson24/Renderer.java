...
class Renderer implements GLEventListener {
    ...
    public void init(GLAutoDrawable drawable) {
        ...
        gl.glBindTexture(GL_TEXTURE_2D, textures[0].texID[0]);        // Select Our Font Texture
    }
    ...
    public void display(GLAutoDrawable drawable) {
        ...
        glPrint(gl, 200, 16, 1, gl.glGetString(GL_RENDERER));       // Display Renderer
        glPrint(gl, 200, 48, 1, gl.glGetString(GL_VENDOR));         // Display Vendor Name
        glPrint(gl, 200, 80, 1, gl.glGetString(GL_VERSION));        // Display Version
        ...
        gl.glBegin(GL_LINE_STRIP);                           // Start Drawing Line Strips (Something New)
        ...
        gl.glBegin(GL_LINE_STRIP);                           // Start Drawing Another Line Strip
        ...
        gl.glEnable(GL_SCISSOR_TEST);                                             // Enable Scissor Testing

        String text = gl.glGetString(GL_EXTENSIONS);           // Allocate Memory For Our Extension String
        ...
        gl.glDisable(GL_SCISSOR_TEST);                         // Disable Scissor Testing
    ...
    private void loadTGA(GL2 gl, TextureImage texture, String filename) throws IOException {        // Loads A TGA File Into Memory
        ByteBuffer TGAcompare = Buffers.newDirectByteBuffer(12);                           // Used To Compare TGA Header
        ...
        try {
            ...
            texture.imageData = Buffers.newDirectByteBuffer(imageSize);                     // Reserve Memory To Hold The TGA Data
            ...
            gl.glBindTexture(GL_TEXTURE_2D, texture.texID[0]);              // Bind Our Texture
            gl.glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR); // Linear Filtered
            gl.glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR); // Linear Filtered

            if (texture.bpp == 24)   // Was The TGA 24 Bits
                type = GL_RGB;   // If So Set The 'type' To GL_RGB

            gl.glTexImage2D(GL_TEXTURE_2D, 0, type, texture.width, texture.height, 0, type, GL_UNSIGNED_BYTE, texture.imageData);
        } finally {
            if (file != null) {
                try {
                    file.close();
    ...
    private void glPrint(GL2 gl, int x, int y, int set, String fmt) {           // Where The Printing Happens
        ...
        if (stringBuffer.capacity() < fmt.length()) {
            stringBuffer = Buffers.newDirectByteBuffer(fmt.length());
        }

        stringBuffer.clear();
        stringBuffer.put(fmt.getBytes());
        stringBuffer.flip();
        gl.glCallLists(fmt.length(), GL_UNSIGNED_BYTE, stringBuffer);    // Write The Text To The Screen
