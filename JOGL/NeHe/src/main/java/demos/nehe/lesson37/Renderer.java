...
class Renderer implements GLEventListener {
    ...
    public void init(GLAutoDrawable drawable) {
        ...
        FloatBuffer shaderData = Buffers.newDirectFloatBuffer(96);                        // Storate For The 96 Shader Values ( NEW )

        // Start Of User Initialization
        gl.glHint(GL_PERSPECTIVE_CORRECTION_HINT, GL_NICEST); // Realy Nice perspective calculations
        ...
        gl.glShadeModel(GL_SMOOTH);                              // Enables Smooth Color Shading ( NEW )
        gl.glDisable(GL_LINE_SMOOTH);                            // Initially Disable Line Smoothing ( NEW )

        gl.glEnable(GL_CULL_FACE);                               // Enable OpenGL Face Culling ( NEW )

        gl.glDisable(GL_LIGHTING);                               // Disable OpenGL Lighting ( NEW )
        ...
        gl.glBindTexture(GL_TEXTURE_1D, shaderTexture[0]);         // Bind This Texture. From Now On It Will Be 1D ( NEW )

        // For Crying Out Loud Don't Let OpenGL Use Bi/Trilinear Filtering! ( NEW )
        gl.glTexParameteri(GL_TEXTURE_1D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
        gl.glTexParameteri(GL_TEXTURE_1D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);

        gl.glTexImage1D(GL_TEXTURE_1D, 0, GL_RGB, 32, 0, GL_RGB, GL_FLOAT, shaderData); // Upload ( NEW )
    ...
    public void display(GLAutoDrawable drawable) {
        ...
        if (outlineSmooth) {                                          // Check To See If We Want Anti-Aliased Lines ( NEW )
            gl.glHint(GL_LINE_SMOOTH_HINT, GL_NICEST);           // Use The Good Calculations ( NEW )
            gl.glEnable(GL_LINE_SMOOTH);                           // Enable Anti-Aliasing ( NEW )
        } else                                                        // We Don't Want Smooth Lines ( NEW )
            gl.glDisable(GL_LINE_SMOOTH);                          // Disable Anti-Aliasing ( NEW )
        ...
        gl.glGetFloatv(GL_MODELVIEW_MATRIX, TmpMatrix.Data, 0);     // Get The Generated Matrix ( NEW )

        // Cel-Shading Code //
        gl.glEnable(GL_TEXTURE_1D);                              // Enable 1D Texturing ( NEW )
        gl.glBindTexture(GL_TEXTURE_1D, shaderTexture[0]);       // Bind Our Texture ( NEW )
        gl.glColor3f(1.0f, 1.0f, 1.0f);                             // Set The Color Of The Model ( NEW )

        gl.glBegin(GL_TRIANGLES);                                // Tell OpenGL That We're Drawing Triangles
        ...
        gl.glDisable(GL_TEXTURE_1D);                             // Disable 1D Textures ( NEW )

        // Outline Code //
        if (outlineDraw) {                                            // Check To See If We Want To Draw The Outline ( NEW )
            gl.glEnable(GL_BLEND);                                 // Enable Blending ( NEW )
            gl.glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);// Set The Blend Mode ( NEW )

            gl.glPolygonMode(GL_BACK, GL_LINE);                  // Draw Backfacing Polygons As Wireframes ( NEW )
            gl.glLineWidth(outlineWidth);                             // Set The Line Width ( NEW )
            gl.glCullFace(GL_FRONT);                               // Don't Draw Any Front-Facing Polygons ( NEW )

            gl.glDepthFunc(GL_LEQUAL);                             // Change The Depth Mode ( NEW )
            gl.glColor3fv(outlineColor, 0);                              // Set The Outline Color ( NEW )

            gl.glBegin(GL_TRIANGLES);                              // Tell OpenGL What We Want To Draw
            ...
            gl.glDepthFunc(GL_LESS);                               // Reset The Depth-Testing Mode ( NEW )
            gl.glCullFace(GL_BACK);                                // Reset The Face To Be Culled ( NEW )
            gl.glPolygonMode(GL_BACK, GL_FILL);                  // Reset Back-Facing Polygon Drawing Mode ( NEW )
            gl.glDisable(GL_BLEND);                                // Disable Blending ( NEW )
        }
