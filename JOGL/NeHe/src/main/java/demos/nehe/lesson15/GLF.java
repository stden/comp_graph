...
*/
    ...
    public void glfDrawSolidSymbol(GL2 gl, char s) {
        ...
        if (contouring == GLF_YES) {
            gl.glGetFloatv(GL_CURRENT_COLOR, temp_color, 0);
...
/* ------------ 3D Wired text drawing ---------------------- */
    ...
    public void glfDraw3DSolidSymbol(GL2 gl, char s) {
        ...
        /* Draw contour, if enabled */
        if (contouring == GLF_YES) {
            gl.glGetBooleanv(GL_LIGHTING, light_temp, 0);
            gl.glDisable(GL_LIGHTING);
            gl.glGetFloatv(GL_CURRENT_COLOR, temp_color, 0);
            gl.glColor4fv(contouring_color, 0);
            glfDraw3DWiredSymbol(gl, s);
            gl.glColor4fv(temp_color, 0);
            if (light_temp[0] != 0) gl.glEnable(GL_LIGHTING);
        }
    }
...
//    int glfLoadBFont(GL2 gl, String fontf) {
...
//
//        /* Build font texture */
//        gl.glBindTexture(GL_TEXTURE_2D, bmf_texture[bmf_curfont]);
//        gl.glTexImage2D(GL_TEXTURE_2D, 0, 3, twidth, theight, 0, GL_RGBA, GL_UNSIGNED_BYTE, texture);
//
...
//
//        /* Build mask texture */
//        gl.glBindTexture(GL_TEXTURE_2D, bmf_mask[bmf_curfont]);
//        gl.glTexImage2D(GL_TEXTURE_2D, 0, 3, twidth, theight, 0, GL_RGBA, GL_UNSIGNED_BYTE, mask);
//        gl.glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
//        gl.glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
//
...
//        for (int i = 0; i < 256; i++) {
//            gl.glNewList(list_base[bmf_curfont] + i, GL_COMPILE);
//
//            gl.glBegin(GL_QUADS);
...
//    int glfUnloadBFontD(GL2 gl, int bmf_descriptor) {
...
//    void glfDrawBSymbol(GL2 gl, char s) {
...
//    void glfDrawBString(GL2 gl, String s)
//    {
//        float temp_trans;
//        int i;
//
...
//        }
//        gl.glCallLists(s.length(), GL_UNSIGNED_BYTE, s.toCharArray());
//        if (m_bitmap_string_center == true) gl.glPopMatrix();
//    }
//
//    void glfDrawBMaskSymbol(GL2 gl, char s) {
//        if ((bmf_curfont < 0) || (bmf_in_use[bmf_curfont] == false)) return;
//
//        gl.glPushMatrix();
...
//    }
//
//    void glfDrawBMaskString(GL2 gl, String s)
//    {
//        float temp_trans;
//        int i;
//
