...
class Renderer implements GLEventListener {
    ...
    private void loadTexture(String fileName, int[] texid, GLAutoDrawable drawable) throws IOException { // Creates Texture From A Bitmap File
        ...
        gl.glPixelStorei(GL_UNPACK_ALIGNMENT, 4); // Pixel Storage Mode (Word Alignment / 4 Bytes)

        // Typical Texture Generation Using Data From The Bitmap
        gl.glBindTexture(GL_TEXTURE_2D, texid[0]);                                // Bind To The Texture ID
        gl.glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);  // Linear Min Filter
        gl.glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);  // Linear Mag Filter
        gl.glTexImage2D(GL_TEXTURE_2D, 0, 3, texture.getWidth(), texture.getHeight(),
                0, GL_BGR, GL_UNSIGNED_BYTE, texture.getPixels());
    }
    ...
    private void drawGLInfo(GL2 gl) {
        ...
        gl.glGetFloatv(GL_PROJECTION_MATRIX, projMatrix, 0); // Grab the projection matrix
        gl.glGetFloatv(GL_MODELVIEW_MATRIX, modelMatrix, 0); // Grab the modelview matrix
