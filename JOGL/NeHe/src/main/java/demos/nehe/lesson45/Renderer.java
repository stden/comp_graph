...
class Renderer implements GLEventListener {
    ...
    public void init(GLAutoDrawable drawable) {
        ...
        gl.glEnable(GL_TEXTURE_2D);                                    // Enable Textures
    ...
    private class Mesh {
        ...
        public boolean loadHeightmap(GL2 gl, String szPath, float flHeightScale, float flResolution, boolean useVBO) throws IOException {
            ...
            gl.glBindTexture(GL_TEXTURE_2D, textureId[0]);                // Bind The Texture
        ...
        public void render(GL2 gl) {
            // Enable Pointers
            gl.glEnableClientState(GL_VERTEX_ARRAY);                        // Enable Vertex Arrays
            gl.glEnableClientState(GL_TEXTURE_COORD_ARRAY);                // Enable Texture Coord Arrays

            // Set Pointers To Our Data
            if (fUseVBO) {
                gl.glBindBuffer(GL_ARRAY_BUFFER, mesh.VBOTexCoords[0]);
                gl.glTexCoordPointer(2, GL_FLOAT, 0, 0);        // Set The TexCoord Pointer To The TexCoord Buffer
                gl.glBindBuffer(GL_ARRAY_BUFFER, mesh.VBOVertices[0]);
                gl.glVertexPointer(3, GL_FLOAT, 0, 0);        // Set The Vertex Pointer To The Vertex Buffer
            } else {
                gl.glVertexPointer(3, GL_FLOAT, 0, mesh.vertices); // Set The Vertex Pointer To Our Vertex Data
                gl.glTexCoordPointer(2, GL_FLOAT, 0, mesh.texCoords); // Set The Vertex Pointer To Our TexCoord Data
            }

            // Render
            gl.glDrawArrays(GL_TRIANGLES, 0, mesh.vertexCount);    // Draw All Of The Triangles At Once

            // Disable Pointers
            gl.glDisableClientState(GL_VERTEX_ARRAY);                    // Disable Vertex Arrays
            gl.glDisableClientState(GL_TEXTURE_COORD_ARRAY);                // Disable Texture Coord Arrays
        }
        ...
        private void buildVBOs(GL2 gl) {
            ...
            gl.glBindBuffer(GL_ARRAY_BUFFER, VBOVertices[0]);            // Bind The Buffer
            // Load The Data
            gl.glBufferData(GL_ARRAY_BUFFER, vertexCount * 3 * Buffers.SIZEOF_FLOAT, vertices, GL_STATIC_DRAW);

            // Generate And Bind The Texture Coordinate Buffer
            gl.glGenBuffers(1, VBOTexCoords, 0);                            // Get A Valid Name
            gl.glBindBuffer(GL_ARRAY_BUFFER, VBOTexCoords[0]);        // Bind The Buffer
            // Load The Data
            gl.glBufferData(GL_ARRAY_BUFFER, vertexCount * 2 * Buffers.SIZEOF_FLOAT, texCoords, GL_STATIC_DRAW);
