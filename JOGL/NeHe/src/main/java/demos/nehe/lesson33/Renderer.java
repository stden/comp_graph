...
class Renderer implements GLEventListener {
    ...
    private void loadGLTextures(GL2 gl) throws IOException                                            // Load Bitmaps And Convert To Textures
    {
        ...
        for (int loop = 0; loop < 2; loop++)                        // Loop Through Both Textures
        {
            ...
            gl.glBindTexture(GL_TEXTURE_2D, texture[loop].texID[0]);
            gl.glTexImage2D(GL_TEXTURE_2D, 0, texture[loop].bpp / 8, texture[loop].width, texture[loop].height, 0, texture[loop].type, GL_UNSIGNED_BYTE, texture[loop].imageData);
    ...
    public void display(GLAutoDrawable glDrawable) {
        ...
        for (int loop = 0; loop < 20; loop++)                            // Loop Of 20
        {
            ...
            gl.glBindTexture(GL_TEXTURE_2D, texture[0].texID[0]);            // ( CHANGE )
            gl.glBegin(GL_QUADS);                                        // Draw Our Quad
            ...
            gl.glBindTexture(GL_TEXTURE_2D, texture[1].texID[0]);            // ( CHANGE )
            gl.glBegin(GL_QUADS);                                        // Draw Our Quad
            gl.glTexCoord2f(0.0f, 0.0f);
            gl.glVertex3f(-1.0f, 1.0f, 0.0f);
            gl.glTexCoord2f(1.0f, 0.0f);
