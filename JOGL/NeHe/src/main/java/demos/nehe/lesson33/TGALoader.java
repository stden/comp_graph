...
import static javax.media.opengl.GL2.GL_RGBA;
...
class TGALoader {
    ...
    static {
        ...
        uTGAcompare = Buffers.newDirectByteBuffer(uncompressedTgaHeader.length);
        uTGAcompare.put(uncompressedTgaHeader);    // Uncompressed TGA Header
        uTGAcompare.flip();

        cTGAcompare = Buffers.newDirectByteBuffer(compressedTgaHeader.length);
    ...
    {
        ByteBuffer header = Buffers.newDirectByteBuffer(12);
    ...
    {                                                                            // TGA Loading code nehe.gamedev.net)
        ...
        texture.imageData = Buffers.newDirectByteBuffer(tga.imageSize);                    // Allocate that much memory
    ...
    private static void loadCompressedTGA(Texture texture, ReadableByteChannel fTGA) throws IOException        // Load COMPRESSED TGAs
    {
        ...
        ByteBuffer colorbuffer = Buffers.newDirectByteBuffer(tga.bytesPerPixel);            // Storage for 1 pixel
