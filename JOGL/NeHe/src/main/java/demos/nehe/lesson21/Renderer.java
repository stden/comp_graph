...
class Renderer implements GLEventListener {
    ...
    void buildFont(GL2 gl) {                                    // Build Our Font Display List
        ...
        for (int i = 0; i < 256; i++)                        // Loop Through All 256 Lists
        {
            ...
            gl.glNewList(base + i, GL_COMPILE);                    // Start Building A List
    ...
    private void glPrint(GL2 gl, int x, int y, int set, String message)    // Where The Printing Happens
    {
        ...
        if (stringBuffer.capacity() < message.length()) {
            stringBuffer = Buffers.newDirectByteBuffer(message.length());
        }

        stringBuffer.clear();
        stringBuffer.put(message.getBytes());
        stringBuffer.flip();
        gl.glCallLists(message.length(), GL_UNSIGNED_BYTE, stringBuffer);        // Write The Text To The Screen
    ...
    public void drawGLScene(GL2 gl) {
        ...
        gl.glBindTexture(GL_TEXTURE_2D, textures[0]);                // Select Our Font Texture
        ...
        for (int loop1 = 0; loop1 < 11; loop1++) {                        // Loop From Left To Right
            for (int loop2 = 0; loop2 < 11; loop2++) {                    // Loop From Top To Bottom
                ...
                gl.glBindTexture(GL_TEXTURE_2D, textures[1]);        // Select The Tile Image
