...
class Renderer implements GLEventListener {
    ...
    private void loadGLTexture(GL2 gl, String name, int[] textures) throws IOException {
    ...
    private static class BezierPatch {                // Structure for a 3rd degree bezier patch (NEW)
        ...
        private int genBezier(GL2 gl, int divs) {
            ...
            gl.glNewList(drawlist, GL_COMPILE);                // Start a new display list
            gl.glBindTexture(GL_TEXTURE_2D, texture[0]);    // Bind the texture
            ...
            for (int u = 1; u <= divs; u++) {
                ...
                gl.glBegin(GL_TRIANGLE_STRIP);                    // Begin a new triangle strip
