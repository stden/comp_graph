...
class Renderer implements GLEventListener {
    ...
    private boolean initMultitexture(GL2 gl) {
        ...
        String extensions;
        extensions = gl.glGetString(GL_EXTENSIONS);                // Fetch Extension String
        ...
        if (multiTextureAvailable != -1             // Is Multitexturing Supported?
            ...
            gl.glGetIntegerv(GL_MAX_TEXTURE_UNITS, maxTexelUnits, 0);
            return true;
        }
    ...
    private void loadGLTextures(GL2 gl, GLU glu) throws IOException {                                   // Load PNGs And Convert To Textures
        ...
        gl.glBindTexture(GL_TEXTURE_2D, textures[0]);        // Create Nearest Filtered Texture
        ...
        gl.glBindTexture(GL_TEXTURE_2D, textures[1]);         // Create Linear Filtered Texture
        gl.glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
        gl.glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
        gl.glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB8, texture.getWidth(), texture.getHeight(), 0, GL_RGB, GL_UNSIGNED_BYTE, texture.getPixels());

        gl.glBindTexture(GL_TEXTURE_2D, textures[2]);         // Create MipMapped Texture
        gl.glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
        gl.glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR_MIPMAP_NEAREST);
        glu.gluBuild2DMipmaps(GL_TEXTURE_2D, GL_RGB8, texture.getWidth(), texture.getHeight(), GL_RGB, GL_UNSIGNED_BYTE, texture.getPixels());
        ...
        gl.glTexParameterfv(GL_TEXTURE_2D, GL_TEXTURE_BORDER_COLOR, gray, 0);

        gl.glGenTextures(3, bump, 0);                              // Create Three Textures

        gl.glBindTexture(GL_TEXTURE_2D, bump[0]);            // Create Nearest Filtered Texture
        gl.glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
        gl.glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
        gl.glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB8, texture.getWidth(), texture.getHeight(), 0, GL_RGB, GL_UNSIGNED_BYTE, texture.getPixels());

        gl.glBindTexture(GL_TEXTURE_2D, bump[1]);            // Create Linear Filtered Texture
        gl.glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
        gl.glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
        gl.glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB8, texture.getWidth(), texture.getHeight(), 0, GL_RGB, GL_UNSIGNED_BYTE, texture.getPixels());

        gl.glBindTexture(GL_TEXTURE_2D, bump[2]);            // Create MipMapped Texture
        gl.glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
        gl.glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR_MIPMAP_NEAREST);
        glu.gluBuild2DMipmaps(GL_TEXTURE_2D, GL_RGB8, texture.getWidth(), texture.getHeight(), GL_RGB, GL_UNSIGNED_BYTE, texture.getPixels());
        ...
        gl.glBindTexture(GL_TEXTURE_2D, invbump[0]);         // Create Nearest Filtered Texture
        gl.glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
        gl.glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
        gl.glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB8, texture.getWidth(), texture.getHeight(), 0, GL_RGB, GL_UNSIGNED_BYTE, texture.getPixels());

        gl.glBindTexture(GL_TEXTURE_2D, invbump[1]);         // Create Linear Filtered Texture
        gl.glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
        gl.glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
        gl.glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB8, texture.getWidth(), texture.getHeight(), 0, GL_RGB, GL_UNSIGNED_BYTE, texture.getPixels());

        gl.glBindTexture(GL_TEXTURE_2D, invbump[2]);         // Create MipMapped Texture
        gl.glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
        gl.glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR_MIPMAP_NEAREST);
        glu.gluBuild2DMipmaps(GL_TEXTURE_2D, GL_RGB8, texture.getWidth(), texture.getHeight(), GL_RGB, GL_UNSIGNED_BYTE, texture.getPixels());
        ...
        ByteBuffer alpha = Buffers.newDirectByteBuffer(texture.getPixels().limit());                            // Create Memory For RGBA8-Texture
        ...
        gl.glBindTexture(GL_TEXTURE_2D, glLogo[0]);                  // Create Linear Filtered RGBA8-Texture
        gl.glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
        gl.glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
        gl.glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA8, texture.getWidth(), texture.getHeight(), 0, GL_RGBA, GL_UNSIGNED_BYTE, alpha);

        texture = TextureReader.readTexture("demos/data/images/Multi_On_Alpha.bmp", true);// Load The "Extension Enabled"-Logo
        alpha = Buffers.newDirectByteBuffer(texture.getPixels().limit());                             // Create Memory For RGBA8-Texture
        ...
        gl.glBindTexture(GL_TEXTURE_2D, multiLogo[0]);               // Create Linear Filtered RGBA8-Texture
        gl.glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
        gl.glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
        gl.glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA8, texture.getWidth(), texture.getHeight(), 0, GL_RGBA, GL_UNSIGNED_BYTE, alpha);
    }
    ...
    void doMesh1TexelUnits(GL2 gl) {
        ...
        gl.glGetFloatv(GL_MODELVIEW_MATRIX, Minv, 0);
        gl.glLoadIdentity();
        gl.glTranslatef(0.0f, 0.0f, z);
    ...
    void doMesh2TexelUnits(GL2 gl) {
        ...
        */
        ...
        gl.glBindTexture(GL_TEXTURE_2D, bump[filter]);
        gl.glTexEnvf(GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_COMBINE);
        gl.glTexEnvf(GL_TEXTURE_ENV, GL_COMBINE_RGB, GL_REPLACE);
        // TEXTURE-UNIT #1:
        gl.glActiveTexture(GL_TEXTURE1);
        gl.glEnable(GL_TEXTURE_2D);
        gl.glBindTexture(GL_TEXTURE_2D, invbump[filter]);
        gl.glTexEnvf(GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_COMBINE);
        gl.glTexEnvf(GL_TEXTURE_ENV, GL_COMBINE_RGB, GL_ADD);
        // General Switches:
        gl.glDisable(GL_BLEND);
        gl.glDisable(GL_LIGHTING);
        gl.glBegin(GL_QUADS);
        ...
        for (i = 0; i < 4; i++) {
            ...
            c[2] = data[5 * i + 4];
            SetUpBumps(n, c, l, s, t);
            gl.glMultiTexCoord2f(GL_TEXTURE0, data[5 * i], data[5 * i + 1]);
            gl.glMultiTexCoord2f(GL_TEXTURE1, data[5 * i] + c[0], data[5 * i + 1] + c[1]);
            gl.glVertex3f(data[5 * i + 2], data[5 * i + 3], data[5 * i + 4]);
        }
