...
var aspect:Double;
begin
    { «начение очистки буфера глубены }
    glClearDepth( 1.0 );

   { разрешение использование тестировани€ глубины }
    glEnable(GL_DEPTH_TEST);

    { установка матрицы проецировани€ }
    glMatrixMode( GL_PROJECTION );
    aspect := (width -panel1.Width-panel2.Width)/ height;
...
begin
 ...
 BackColor:=BackColorShape.Brush.Color;
 { начально положение глаза наблюдател€ }
...
begin
  ...
  wglMakeCurrent(dc, hglrc);
  { установка новой области отображени€ OpenGl }
  glViewport(GLsizei(Panel1.Width+1),0,
             GLsizei(ClientWidth-Panel1.Width-2),
             GLsizei(ClientHeight-2));

  { вычисление нового соотвношени€ ширены к высоте видемой области }
  aspect :=  (ClientWidth-Panel1.Width-2) / (Clientheight-2);

  { соответствующее изменение матрицы проецировани€ }
  glMatrixMode( GL_PROJECTION );
  glLoadIdentity();
  gluPerspective( 45, aspect, 2.0, 10.0 );

  glMatrixMode( GL_MODELVIEW );
  wglMakeCurrent(0, 0);
  ReleaseDC(wnd, dc);
  { вызов процедуры рисовани€ сцены }
  Draw;
end;
...
begin
  ...
  if cbAlpha.Checked then begin   { ≈сли тестирование альфа-канала включено }
    glEnable(GL_ALPHA_TEST);       { –азрешить тестирование в OpenGL}
    ref:=StrToFloat(edtRef.Text);  { ѕолучить значение REF из соответствующего пол€ ввода}
    glAlphaFunc(AlphaFuncs[cbAlphaFunc.ItemIndex], ref); { ”стоновить нужную функцию сравнени€ }
  end else
    glDisable(GL_ALPHA_TEST);  { иначе запрещаем тестирование альфа-канал в OpenGL }

  // ----------------------------------------------------------------------------
  if cbBlend.Checked then begin { ≈сли смешивание цветов включено }
    glEnable(GL_BLEND);          { –азрешаем смешивание в OpenGL }
    glBlendFunc(SFactors[cbSFactor.ItemIndex],  { ”станавливаем параметры }
                DFactors[cbDFactor.ItemIndex]);
  end else
    glDisable(GL_BLEND); { «апрещаем смешивание }

  glDisable(GL_SCISSOR_TEST); { «апрет "вырезки" в цел€х очистки всего окна }
 ...
 { построенние матриц высот и составл€ющих вектора нормали в
   матрица квадратна€ размера GridCount X GridCount}
 step:=1/GridSize; y:=0;
 for i:=0 to GridSize-1 do begin
  ...
  for j:=0 to GridSize-1 do with z[i,j] do begin
   ...
   nx:=nx/len; { состовл€ющие новрмали в данной точке }
   ny:=ny/len;
   nz:=1.0/len;
   x:=x+step;
  end;
 ...
 { аппроксимаци€ поверхности треугольниками }
 glBegin(gl_Triangles);
 y:=-1; step:=step*2;
  for i:=0 to GridSize-2 do begin
    x:=-1;
    for j:=0 to GridSize-2 do begin
      with z[i,j] do begin glNormal3d(nx,ny,nz); glVertex4f(x,y,val,0.5); end;
      with z[i,j+1] do begin glNormal3d(nx,ny,nz); glVertex4f(x+step,y,val,0.5); end;
      with z[i+1,j] do begin glNormal3d(nx,ny,nz); glVertex4f(x,y+step,val,0.5); end;

      with z[i+1,j] do begin glNormal3d(nx,ny,nz); glVertex4f(x,y+step,val,0.5); end;
      with z[i,j+1] do begin glNormal3d(nx,ny,nz); glVertex4f(x+step,y,val,0.5); end;
      with z[i+1,j+1] do begin glNormal3d(nx,ny,nz); glVertex4f(x+step,y+step,val,0.5); end;
...
begin
  ...
  BackColorShape.Brush.Color:=BackColor;
  if not cbAnimation.Checked then Draw;
end;
